...
(async () => {
    const tokens = (process.argv[2] || process.env.FEISHU_INSPECT_TOKENS || '').split(',').filter(Boolean);
    if (tokens.length === 0) {
        console.error('Usage: node inspect_meta.js <appToken1,appToken2,...>');
        console.error('Or set FEISHU_INSPECT_TOKENS env var');
        process.exit(1);
    }
    for (const t of tokens) {
        await inspect(t.trim(), t.trim());
    }
})();
